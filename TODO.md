...
- **Tests**: 38,689 passing (workspace-wide, `--all-features`), 37,346 with default features, 2 skipped (GPU-only on macOS)
...
- [~] **Alt-backend audit** (oxicuda-vulkan / metal / rocm / levelzero / webgpu) — never ran (session token limit); re-run the alt-backend audit next session.
  - **Status (2026-08-12):** `oxicuda-metal` and `oxicuda-webgpu` audited and fixed on real Apple M3 hardware (Metal 4, macOS, arm64) — see the "Remaining GAP" summary above (~line 29) and `CHANGELOG.md` `[0.5.5]` for the full writeup. `oxicuda-metal`: 386/386 tests passing (`--all-features`), clippy-clean. `oxicuda-webgpu`: 277/278 (one known test-file bug, not a backend defect — see the follow-up below).
...
- 277 of 278 tests passing (measured 2026-08-12, `--all-features`; the one failure is a bug in the newly-added `tests/gpu_presence.rs` integration test itself — an aliased input/output pointer that the backend correctly rejects — not a backend defect; see follow-ups). Count moves quickly — re-run `cargo nextest run -p oxicuda-webgpu --all-features` for the current figure.
...
- 386 tests passing (measured 2026-08-12, `--all-features`; concurrent work in this crate means this count moves quickly — re-run `cargo nextest run -p oxicuda-metal --all-features` for the current figure)
...
- [ ] **`oxicuda-webgpu` `tests/gpu_presence.rs::webgpu_backend_init_must_succeed` fails, and it exposes an undocumented cross-backend contract gap** — the test dispatches `unary(Relu, ptr, ptr, 4)` with the *same* handle as `input_ptr` and `output_ptr` (in-place). `WebGpuBackend` correctly rejects this (`InvalidArgument`: wgpu forbids binding one buffer as both `read` and `read_write` in one dispatch). But `oxicuda-metal`'s **own** sibling test (`tests/gpu_presence.rs::metal_backend_init_must_succeed`) does the *exact same* in-place `unary(Relu, ptr, ptr, 4)` call and **passes**, because `MetalBackend::unary` has no aliasing check at all. `ComputeBackend::unary`'s trait doc does not say whether `input_ptr == output_ptr` is allowed — Metal silently permits it, WebGPU silently forbids it. Immediate fix: give the webgpu test a separate output buffer (a portable caller must never rely on in-place aliasing anyway). Longer-term: document the aliasing contract on the trait itself, and decide whether Metal should also reject it for cross-backend consistency.
