//! WebGPU scaled dot-product attention benchmark — measures
...
//!
//! This exercises the GPU dispatch path (`shader::attention_wgsl`), not the
//! CPU-fallback path, since batch_heads × seq_q = 8×256 = 2048 is well
//! within the fixed 1-D dispatch grid `attention_gpu_dispatch_grid` supports.
//!
//! ## Platform behaviour
//!
//! * **A machine with a usable wgpu adapter** — the benchmark runs.
//! * **No adapter (headless CI)** — prints `skip: no WebGPU adapter
//!   (attention)` to stderr and returns before registering any benchmark
//!   closure with criterion.
//!
//! Run with:
//! ```bash
//! cargo bench -p oxicuda-webgpu --bench attention_bench
//! ```
...
fn bench_attention(criterion: &mut Criterion) {
    let harness = match try_setup() {
        Some(h) => h,
        None => {
            eprintln!("skip: no WebGPU adapter (attention)");
            return;
        }
    };
    ...
    let mut group = criterion.benchmark_group("webgpu_attention_b1_h8_s256_d64_causal");
